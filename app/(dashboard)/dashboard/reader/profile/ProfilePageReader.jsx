...
export default function ProfilePageReader({ name, role, image }) {
    ...
    // If an image prop is passed in, start in "uploaded" mode so it renders immediately
    const [activeAvatarType, setActiveAvatarType] = useState(image ? "uploaded" : "preset");
    const [selectedPresetId, setSelectedPresetId] = useState("fuji");
    const [uploadedImageSrc, setUploadedImageSrc] = useState(image || null);
    ...
    const handleImageUploadChange = async (e) => {
        ...
        try {
            ...
            if (result.success && result.data && result.data.url) {
                // ✅ FIX: capture the URL in a local variable first, then use it for
                // both setState and authClient — avoids reading stale closure state
                const newImageUrl = result.data.url;

                setUploadedImageSrc(newImageUrl);
    ...
    return (
        <div className="min-h-screen bg-paper text-ink p-4 md:p-8 max-w-[1280px] mx-auto space-y-8 select-none relative">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                {/* Left Column (5 Cols): The Hanko Seal Showcase Card */}
                    ...
                    <div className="card-ink p-6 bg-paper flex flex-col items-center justify-center text-center relative overflow-hidden">
                        ...
                        <div className="w-40 h-40 rounded-full border-4 border-ink p-1 flex items-center justify-center bg-paper shadow-ink-sm relative group mt-4">
                            <div className="w-full h-full rounded-full border-2 border-dashed border-ink/40 overflow-hidden flex items-center justify-center bg-paper relative">
                                ...
                                ) : activeAvatarType === "uploaded" && uploadedImageSrc ? (
                                    <Image
                                        width={160}
                                        height={160}
                ...
                {/* Right Column (7 Cols): Information Adjustment Form Card */}
