...
const MOCK_EBOOKS_DATA = [
    {
        "_id": { "$oid": "6a37a6907c2e396411353086" },
        "title": "The Scarlet Hat",
        "writerName": "Albert Dera",
        "writerEmail": "albert.dera@fable.com",
        "writerId": "6a351b6360207f18529ce973",
        "description": "A striking historical fiction mystery tracking a secret message left behind a painting in a Paris art studio.",
        "price": 14.99,
        "genre": "Fiction",
        "coverImage": "https://i.ibb.co.com/KHkFbtq/girl-with-red-hat-o-Rv-Snk49-R6k-unsplash.jpg",
        "status": "Published",
        "purchaseCount": 24,
        "rating": 4.5,
    ...
    {
        "_id": { "$oid": "6a37a6907c2e396411353087" },
        "title": "Bamboo Forest Whispers",
        "writerName": "Master Basho",
        "writerEmail": "basho@fable-revival.jp",
        "writerId": "6a351b6360207f18529ce974",
        "description": "An intimate collection of haikus and poetic essays chronicling travels through the silent northern paths.",
        "price": 8.50,
        "genre": "Waka Poetry",
        "coverImage": "https://images.unsplash.com/photo-1504618223053-559bdef9dd5a?auto=format&fit=crop&q=80&w=200",
        "status": "Published",
        "purchaseCount": 42,
        "rating": 4.9,
    ...
    {
        "_id": { "$oid": "6a37a6907c2e396411353088" },
        "title": "Tales of the Ghostly Lantern",
        "writerName": "Hokusai Katsushika",
        "writerEmail": "hokusai@fable-revival.jp",
        "writerId": "6a351b6360207f18529ce975",
        "description": "Eerie accounts of local folklore and spirits compiled during restless autumn nights under a waning crescent.",
        "price": 11.00,
        "genre": "Folklore",
        "coverImage": "https://images.unsplash.com/photo-1534447677768-be436bb09401?auto=format&fit=crop&q=80&w=200",
        "status": "Unpublished",
        "purchaseCount": 0,
        "rating": 0,
    ...
    {
        "_id": { "$oid": "6a37a6907c2e396411353089" },
        "title": "The Scribe's Hidden Inkwell",
        "writerName": "Murasaki Shikibu",
        "writerEmail": "shikibu@fable-revival.jp",
        "writerId": "6a351b6360207f18529ce976",
        "description": "Courtyard secrets and silent romances of the ancient imperial palaces penned down on delicate silk panels.",
        "price": 14.50,
        "genre": "History Scrolls",
...
export default function AdminEbooksPage() {
    const [ebooks, setEbooks] = useState(MOCK_EBOOKS_DATA);
        ...
        <div className="min-h-screen bg-paper text-ink p-4 md:p-8 max-w-[1280px] mx-auto space-y-8 select-none">
            ...
            <section className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div className="card-ink p-5 bg-paper">
                    ...
                    <div className="flex justify-between items-baseline mt-2">
                        <h3 className="text-3xl font-bold font-display">
                            {ebooks.filter(b => b.status === "Unpublished").length}
                        </h3>
                        ...
                        <tbody>
                            {filteredEbooks.length === 0 ? (
                                ...
                                filteredEbooks.map((book) => (
                                    <tr key={book._id.$oid} className="border-b-2 border-ink/15 hover:bg-ink/5 transition-colors">
                                        ...
                                        {/* Status Badge Column */}
                                        <td className="px-6 py-4 border-r-2 border-ink/10 bg-transparent">
                                            <span className={`inline-block text-[9px] font-extrabold uppercase px-2.5 py-1 rounded-md border-2 ${book.status === "Published"
                                        ...
                                        <td className="px-6 py-4 bg-transparent">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button
                                                    onClick={() => handleToggleStatus(book._id.$oid)}
                                                    type="button"
                                                    className={`px-3 py-1.5 border-2 border-ink rounded-lg font-display text-[9px] font-bold uppercase tracking-wider transition-all cursor-pointer shadow-ink-sm active:translate-y-px ${book.status === "Published"
                                                        ? "bg-ochre text-paper"
                                                        : "bg-sun text-paper"
                                                        }`}
                                                    title={book.status === "Published" ? "Retract manuscript" : "Expose manuscript"}
                                                >
                                                    <span className="flex items-center gap-1">
                                                        {book.status === "Published" ? <EyeClosedIcon /> : <EyeOpenIcon />}
                                                        <span>{book.status === "Published" ? "Unpublish" : "Publish"}</span>
                                                    </span>
                                                </button>

                                                {/* Trash/Delete Button */}
                                                <button
                                                    onClick={() => handleDeleteEbook(book._id.$oid)}
                                                    type="button"
            ...
            <section className="block md:hidden space-y-5">
                {filteredEbooks.length === 0 ? (
                    ...
                    filteredEbooks.map((book) => (
                        <div key={book._id.$oid} className="card-ink p-5 bg-paper flex flex-col gap-4 relative">
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="space-y-2 min-w-0 flex-1">
                                    <div className="flex items-center justify-between gap-2 flex-wrap">
                                        ...
                                        <span className={`inline-block text-[9px] font-extrabold uppercase px-2.5 py-0.5 rounded border-2 ${book.status === "Published"
                                            ? "bg-prussian/10 text-prussian border-prussian"
                                            : "bg-ochre/10 text-ochre border-ochre"
                                            }`}>
                                            {book.status}
                            ...
                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-1">
                                ...
                                <div className="flex items-center gap-2 justify-between">
                                    <button
                                        onClick={() => handleToggleStatus(book._id.$oid)}
                                        type="button"
                                        className={`px-3 py-2 border-2 border-ink rounded-lg font-display text-[9px] font-bold uppercase tracking-wider transition-all cursor-pointer shadow-ink-sm flex-1 text-center flex items-center justify-center gap-1.5 ${book.status === "Published"
                                            ? "bg-ochre text-paper"
                                            : "bg-sun text-paper"
                                            }`}
                                    >
                                        {book.status === "Published" ? <EyeClosedIcon /> : <EyeOpenIcon />}
                                        <span>{book.status === "Published" ? "Unpublish Scroll" : "Publish Scroll"}</span>
                                    </button>
